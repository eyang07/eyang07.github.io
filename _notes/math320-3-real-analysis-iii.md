---
layout: page
title: Real Analysis III
course_code: MATH 320-3
department: Mathematics
permalink: /notes/math320-3-real-analysis-iii/
---

## Real Analysis III Notes

Typeset from the Midterm 1, Midterm 2, and Final handwritten PDF sources with corrected scan-level spelling and a cleaner academic style.

<p class="handwritten-sources">
  Handwritten sources:
  <a href="{{ '/assets/pdf/notes/original/math320-3-midterm-1-handwritten.pdf' | relative_url }}" target="_blank">Midterm 1</a>
  <a href="{{ '/assets/pdf/notes/original/math320-3-midterm-2-handwritten.pdf' | relative_url }}" target="_blank">Midterm 2</a>
  <a href="{{ '/assets/pdf/notes/original/math320-3-final-handwritten.pdf' | relative_url }}" target="_blank">Final</a>
</p>

{% include note-viewer.liquid
  title="MATH 320-3 Real Analysis III Notes"
  pdf="/assets/pdf/notes/math320-3-real-analysis-iii-notes.pdf"
  tex="/assets/tex/notes/math320-3-real-analysis-iii-notes.tex"
%}
